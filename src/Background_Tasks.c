 ...
 */
 ...
 /// @brief This task regularly checks the stop button and arm limit switches.
 ...
 void checkSensors(void *param)
 {
     ...
     while (1)
     {
         ...
         if (adi_digital_read(_armLimitLow))		   // Check lower limit switch
         {
             ...
             if (arm_motorDir < 0)
             {
                 motor_move(_motorArm, 0); 		// stop if commanded to lower the arm
             }
         }
         else
         {
             if (adi_digital_read(_armLimitHigh))
             {
                 ...
                 if (arm_motorDir > 0)
                 {
                     motor_move(_motorArm, 0); 	// stop if commanded to raise the arm
                 }
